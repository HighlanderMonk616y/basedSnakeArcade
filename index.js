...
function createTrailParticle(x, y) {
  ...
  const life = isFever ? 28 : 18;
  let color = '#0f0';
  
  if (isFever) {
    const hue = (Date.now() / 8 + Math.random() * 40) % 360;
...
function draw() {
  ...
  // Power-up flash
  if (powerUpFlash > 0) {
    ctx.fillStyle = `rgba(255, 255, 200, ${powerUpFlash / 25})`;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    powerUpFlash--;
  }
  ...
  // Points to beat high score
  if (score < highScore) {
    const needed = highScore - score;
    ctx.fillStyle = '#ff8';
    ctx.font = '12px monospace';
    ctx.textAlign = 'right';
    ctx.fillText(`${needed} to HIGH`, canvas.width - 10, 140);
  } else if (score > highScore) {
    ctx.fillStyle = '#0f0';
    ctx.font = '12px monospace';
    ctx.textAlign = 'right';
    ctx.fillText('NEW HIGH!', canvas.width - 10, 140);
  }
  ...
  if (combo === 6 || combo === 7) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('FEVER SOON', canvas.width/2, 48);
    ctx.globalAlpha = 1;
  }
